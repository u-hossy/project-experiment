...
export default function BillingDetailCard({ payer }: BillingDetailCardProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const payerBilling = billingData.filter((b: Billing) => b.payerId === payer.id)
  ...
  const handleBlur = async (index: number) => {
    ...
    if (!d.receiverId || d.amount === "") {
      if (d.id) {
        await fetch(`http://localhost:3001/billings/${d.id}`, { method: "DELETE" })
      }
    ...
    if (index === details.length - 1) {
      setDetails(prev => [...prev, { id: crypto.randomUUID(), receiverId: "", amount: "" }])
    }
  }
